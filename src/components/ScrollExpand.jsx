...
const ScrollExpand = ({
  ...
  useEffect(() => {
    ...
    if (propsRef.current.autoPlay) {
      let rafId = 0;
      let start = null;
      const duration = 900; // 0.9s duration matching the layout morph
      
      measure();
      
      // Auto-play overrides the track height so it doesn't create extra scrolling
      ...
      window.addEventListener('resize', onResizeAuto);
      
      return () => {
        if (rafId) cancelAnimationFrame(rafId);
        window.removeEventListener('resize', onResizeAuto);
      };
    }
  ...
  const media =
    mediaType === 'video' ? (
      ...
      <motion.img layoutId={layoutId} ref={mediaRef} className="scroll-expand__media" src={src} alt={alt} draggable={false} />
    ) : (
